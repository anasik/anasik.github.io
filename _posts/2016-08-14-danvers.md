...
The design end is more or less identical to what quicksites offered. There&#8217;s a themes folder and you need to edit a file to change the theme. But there has been a slight addition to the number of _required_ files. There are a number of template files, like for the singlepost pages or for comment forms, and they use certain tags to define where the dynamically loaded content should be present, but it is all very intuitive.
