...
categories:
  ...
  - Tech
---
You are using a computing device, be it a smartphone, a tablet, a desktop computer. It&#8217;s new, shiny, with little or no applications installed, _apart from the bloatware that the manufacturer could have generously shipped with it._ You fire up Facebook in a web browser, like a couple of pictures, post a status, have a small chat with a friend, and then after a while, you close the tab and lock your phone. After a while you do it again, and this time, you spend a whole hour scrolling through the news feed, and then once again you close the tab, and lock your device.
...
And _that,_ is what the idea of _&#8220;Progressive Web Apps&#8221;_ is. Yes, the name is pretty straight forward. Facebook, an app on the web, progressed for you, from a tab in your default browser, through a link on your homescreen, to an app running natively on your device capable of running in the background and sending push notifications. The beauty of it? the user never had to fire up an app-store or download a package file in order to install it. The computer itself computed that the user liked using the web-app enough to want to have a standalone app.
...
So now that we have a decent idea of what user-experience with PWAs could be like, let&#8217;s move on to the next obvious topic. _Development._ The questions that come to one&#8217;s head when they try visualizing the experience, and my answers to them are as follows:
